      ...
      >
        {links.map((l) => (
          <motion.div key={l.name} variants={navItemVariants}>
            <Link
              href={l.link}
              className={cn("capitalize", pathname == l.link && "text-primary")}
            >
              {l.name}
            </Link>
          </motion.div>
      ...
      <AnimatePresence>
        {open && (
          <motion.nav
            ...
            className={cn(
              "absolute top-full inset-x-0 rounded-lg mt-2 py-4 bg-white/60 backdrop-blur-md w-full flex flex-col items-center gap-8 text-[#808282] shadow-[0px_0px_4px_rgba(251,196,26,0.3),0px_0px_8px_rgb(251,196,26,0.1)]"
            )}
          >
            {links.map((l) => (
              <Link
